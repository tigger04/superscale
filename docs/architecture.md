...
4. **CoreMLInference** — loads the `.mlpackage` model via `ModelCache`, creates a `VNCoreMLRequest`, runs each tile through the model. The model outputs a tile at `scale×` the input resolution. CoreML automatically dispatches to the Neural Engine, GPU, or CPU depending on hardware and model compatibility. Tiles smaller than the model's expected input (512×512) are reflection-padded to full size before inference, then the output is cropped back to the correct dimensions. The padding uses a flipped CGContext (top-left origin) to ensure correct orientation in the output CGImage.
